...
static void observe_mode(int mode) {
    g_last_mode = mode;
    g_restore_required = mode != g_original_mode;
...
int color_space_apply_config(void) {
    ...
    int ret = resolve_locked();
    if (ret < 0) {
        if (!requested && !g_restore_required) {
            status_clear_error_domain(VBE_ERROR_DOMAIN_COLOR_SPACE);
            return 0;
        }
        color_error(ret);
...
int color_space_shutdown(void) {
    ...
    if (g_get_mode == NULL || g_set_mode == NULL || !valid_mode(g_original_mode)) {
        status_set_capability(VBE_CAP_FAILED);
        color_error(-1);
        return -1;
    }
    ...
    if (current != g_original_mode) {
        int ret = g_set_mode(g_original_mode);
        if (ret < 0) {
            status_set_capability(VBE_CAP_FAILED);
            color_error(ret);
            return ret;
        ...
        current = g_get_mode();
        if (!valid_mode(current) || current != g_original_mode) {
            int detail = current < 0 ? current : -(0x300 + current);
            status_set_capability(VBE_CAP_FAILED);
            color_error(detail);
            return detail ? detail : -1;
        }
