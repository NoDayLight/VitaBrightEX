...
static tai_hook_ref_t g_ref_csc_a, g_ref_csc_b;
static tai_hook_ref_t g_ref_display_brightness, g_ref_display_colorspace;
static tai_hook_ref_t g_ref_lcd_brightness, g_ref_lcd_colorspace;
static tai_hook_ref_t g_ref_display_on, g_ref_display_off;
static tai_hook_ref_t g_ref_iftu_enable;
static tai_hook_ref_t g_ref_panel_write, g_ref_panel_read;

static SceUID g_hook_csc_a = -1, g_hook_csc_b = -1;
static SceUID g_hook_display_brightness = -1, g_hook_display_colorspace = -1;
static SceUID g_hook_lcd_brightness = -1, g_hook_lcd_colorspace = -1;
static SceUID g_hook_display_on = -1, g_hook_display_off = -1;
static SceUID g_hook_iftu_enable = -1;
static SceUID g_hook_panel_write = -1, g_hook_panel_read = -1;
...
static int exact_signature(const volatile uint8_t *address,
...
static SceUID install_export_hook(tai_hook_ref_t *ref, const char *module,
                                  uint32_t nid, const void *hook,
                                  uint32_t hook_bit, uint32_t fail_bit) {
...
static int setup_lcd_private_paths(void) {
    ...
    uintptr_t state = 0, writer = 0, reader = 0;
    int ret;
    lcd.size = sizeof(lcd);
    ret = taiGetModuleInfoForKernel(KERNEL_PID, "SceLcd", &lcd);
    if (ret < 0) {
        g_hook_fail_mask |= VBE_TRACE_FAIL_PANEL_WRITE |
                            VBE_TRACE_FAIL_PANEL_READ |
                            VBE_TRACE_FAIL_LCD_SNAPSHOT;
        return ret;
    }

    ret = module_get_offset(KERNEL_PID, lcd.modid, VBE_LCD_STATE_SEGMENT,
                            VBE_LCD_STATE_OFFSET, &state);
    if (ret >= 0) {
        g_lcd_state_base = state;
        g_snapshot_available_mask |= VBE_TRACE_SNAPSHOT_LCD;
    } else {
        g_hook_fail_mask |= VBE_TRACE_FAIL_LCD_SNAPSHOT;
    }

    ret = module_get_offset(KERNEL_PID, lcd.modid, VBE_LCD_PANEL_SEGMENT,
                            VBE_LCD_PANEL_WRITER_OFFSET, &writer);
    if (ret < 0 ||
        !exact_signature((const volatile uint8_t *)writer,
                         vbe_lcd_panel_writer_signature,
                         VBE_LCD_PRIVATE_SIGNATURE_LENGTH)) {
        g_hook_fail_mask |= VBE_TRACE_FAIL_PANEL_WRITE_SIG;
    } else {
        g_hook_panel_write = taiHookFunctionOffsetForKernel(
            KERNEL_PID, &g_ref_panel_write, lcd.modid, VBE_LCD_PANEL_SEGMENT,
            VBE_LCD_PANEL_WRITER_OFFSET, 1, hook_panel_write);
        if (g_hook_panel_write < 0) {
            g_hook_panel_write = -1;
            g_hook_fail_mask |= VBE_TRACE_FAIL_PANEL_WRITE;
        } else {
            g_installed_hook_mask |= VBE_TRACE_HOOK_PANEL_WRITE;
        }
    }

    ret = module_get_offset(KERNEL_PID, lcd.modid, VBE_LCD_PANEL_SEGMENT,
                            VBE_LCD_PANEL_READER_OFFSET, &reader);
    if (ret < 0 ||
        !exact_signature((const volatile uint8_t *)reader,
                         vbe_lcd_panel_reader_signature,
                         VBE_LCD_PRIVATE_SIGNATURE_LENGTH)) {
        g_hook_fail_mask |= VBE_TRACE_FAIL_PANEL_READ_SIG;
    } else {
        g_hook_panel_read = taiHookFunctionOffsetForKernel(
            KERNEL_PID, &g_ref_panel_read, lcd.modid, VBE_LCD_PANEL_SEGMENT,
            VBE_LCD_PANEL_READER_OFFSET, 1, hook_panel_read);
        if (g_hook_panel_read < 0) {
            g_hook_panel_read = -1;
            g_hook_fail_mask |= VBE_TRACE_FAIL_PANEL_READ;
        } else {
            g_installed_hook_mask |= VBE_TRACE_HOOK_PANEL_READ;
        }
    }
    return 0;
}
...
static void clear_records(void) {
    ...
    for (i = 0; i < VBE_TRACE_RECORD_CAPACITY; ++i) {
        ...
        for (j = 0; j < (uint32_t)(sizeof(VbeTraceRecord) / sizeof(uint32_t)); ++j)
            w[j] = 0;
    }
}
...
static uint32_t count_committed_records(uint32_t slots) {
    ...
    for (i = 0; i < limit; ++i)
        if (g_records[i].committed == VBE_TRACE_COMMITTED) ++count;
    return count;
}

static void snapshot_data_read(VbeTraceSnapshotData *data, uint32_t available_mask) {
    uint32_t i;
    zero_bytes(data, (uint32_t)sizeof(*data));
    if ((available_mask & VBE_TRACE_SNAPSHOT_LOWIO) != 0u) {
        for (i = 0; i < VBE_TRACE_PLANE_COUNT; ++i) {
            const volatile uint8_t *p = (const volatile uint8_t *)(g_plane_base +
                                         i * LOWIO_PLANE_STRIDE);
...
int vbeTraceGetStatus(VbeTraceStatus *out) {
    VbeTraceStatus s;
    uint32_t cpu_state, slots = g_slots_reserved;
    int ret;
    ...
    s.committed_records = count_committed_records(slots);
    s.lost_records = g_lost;
    s.last_sequence = g_sequence;
    s.active_hooks = g_active_hooks;
    s.installed_hook_mask = g_installed_hook_mask;
    s.required_hook_mask = VBE_TRACE_REQUIRED_HOOKS;
...
int vbeTraceSnapshot(VbeTraceSnapshot *out) {
    VbeTraceSnapshotData a, b;
    VbeTraceSnapshot snap;
    uint32_t cpu_state, available;
    int ret;
    if (out == NULL) return VBE_TRACE_ERR_INVALID;
    if (g_firmware_version != VBE_TRACE_FW_365) return VBE_TRACE_ERR_UNAVAILABLE;
    if (g_enabled != 0u) return VBE_TRACE_ERR_BUSY;
    __sync_synchronize();
    if (g_active_hooks != 0u) return VBE_TRACE_ERR_BUSY;
    available = g_snapshot_available_mask;
    snapshot_data_read(&a, available);
    __sync_synchronize();
    snapshot_data_read(&b, available);
    __sync_synchronize();
    if (g_enabled != 0u || g_active_hooks != 0u) return VBE_TRACE_ERR_BUSY;
...
static int release_owned_hook(SceUID *uid, tai_hook_ref_t ref, uint32_t hook_bit) {
    int ret;
    if (*uid < 0) return 0;
    ret = taiHookReleaseForKernel(*uid, ref);
    if (ret < 0) return ret;
    *uid = -1;
    g_installed_hook_mask &= ~hook_bit;
    return 0;
...
int module_start(SceSize argc, const void *args) {
    ...
    g_active_hooks = 0;
    g_hook_fail_mask = 0;
    g_installed_hook_mask = 0;
    g_snapshot_available_mask = 0;
    g_plane_base = 0;
    g_lcd_state_base = 0;
    clear_records();
    ...
    g_hook_csc_a = install_export_hook(&g_ref_csc_a, "SceLowio", NID_IFTU_CSC_A,
        hook_csc_a, VBE_TRACE_HOOK_CSC_A, VBE_TRACE_FAIL_CSC_A);
    g_hook_csc_b = install_export_hook(&g_ref_csc_b, "SceLowio", NID_IFTU_CSC_B,
        hook_csc_b, VBE_TRACE_HOOK_CSC_B, VBE_TRACE_FAIL_CSC_B);
    g_hook_iftu_enable = install_export_hook(&g_ref_iftu_enable, "SceLowio", NID_IFTU_ENABLE,
        hook_iftu_enable, VBE_TRACE_HOOK_IFTU_ENABLE, VBE_TRACE_FAIL_IFTU_ENABLE);
    g_hook_display_brightness = install_export_hook(&g_ref_display_brightness, "SceDisplay",
        ...
        VBE_TRACE_HOOK_DISPLAY_BRIGHT, VBE_TRACE_FAIL_DISPLAY_BRIGHT);
    g_hook_display_colorspace = install_export_hook(&g_ref_display_colorspace, "SceDisplay",
        ...
        VBE_TRACE_HOOK_DISPLAY_COLOR, VBE_TRACE_FAIL_DISPLAY_COLOR);
    g_hook_lcd_brightness = install_export_hook(&g_ref_lcd_brightness, "SceLcd",
        NID_LCD_BRIGHTNESS, hook_lcd_brightness,
        VBE_TRACE_HOOK_LCD_BRIGHT, VBE_TRACE_FAIL_LCD_BRIGHT);
    g_hook_lcd_colorspace = install_export_hook(&g_ref_lcd_colorspace, "SceLcd",
        NID_LCD_COLORSPACE, hook_lcd_colorspace,
        VBE_TRACE_HOOK_LCD_COLOR, VBE_TRACE_FAIL_LCD_COLOR);
    g_hook_display_on = install_export_hook(&g_ref_display_on, "SceLcd", NID_LCD_DISPLAY_ON,
        hook_display_on, VBE_TRACE_HOOK_DISPLAY_ON, VBE_TRACE_FAIL_DISPLAY_ON);
    g_hook_display_off = install_export_hook(&g_ref_display_off, "SceLcd", NID_LCD_DISPLAY_OFF,
        hook_display_off, VBE_TRACE_HOOK_DISPLAY_OFF, VBE_TRACE_FAIL_DISPLAY_OFF);
