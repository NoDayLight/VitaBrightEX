...
static uint32_t annotate_panel_read_validity(VbeTraceRecord *records, uint32_t count) {
    uint32_t i, saw_proven = 0u, saw_uncertain = 0u;
    for (i = 0; i < count; ++i) {
        VbeTraceRecord *r = &records[i];
        if (r->event_type != VBE_TRACE_PANEL_READ_EXIT || r->raw_return != 0 ||
            (r->flags & VBE_TRACE_FLAG_NULL) != 0u) continue;
        if (r->arg0 == 0x0Au && r->arg1 == 1u && r->payload_length == 1u &&
    ...
    return saw_proven ? VBE_TRACE_PANEL_READ_PROVEN_ONLY : VBE_TRACE_PANEL_READ_NONE;
}
...
int main(void) {
    ...
    uint32_t record_count = 0u, post_retry_count = 0u, truncated_seen = 0u;
    uint32_t common_snapshot_mask, read_validity;
    ...
    header.affine_missing_required_mask = VBE_TRACE_REQUIRED_AFFINE_HOOKS & ~status.installed_hook_mask;
    header.panel_required_mask = VBE_TRACE_REQUIRED_PANEL_HOOKS;
    header.panel_missing_required_mask = VBE_TRACE_REQUIRED_PANEL_HOOKS & ~status.installed_hook_mask;
    header.affine_required_snapshot_mask = VBE_TRACE_REQUIRED_AFFINE_SNAPSHOTS;
    header.affine_missing_required_snapshot_mask =
        VBE_TRACE_REQUIRED_AFFINE_SNAPSHOTS & ~common_snapshot_mask;
    header.panel_required_snapshot_mask = VBE_TRACE_REQUIRED_PANEL_SNAPSHOTS;
    ...
    header.affine_capture_quality =
        (pre_present && header.affine_missing_required_mask == 0u &&
         header.affine_missing_required_snapshot_mask == 0u && status.lost_records == 0u) ?
    ...
    header.panel_capture_quality =
        (pre_present && header.panel_missing_required_mask == 0u &&
         header.panel_missing_required_snapshot_mask == 0u && status.lost_records == 0u &&
         truncated_seen == 0u && read_validity != VBE_TRACE_PANEL_READ_HAS_UNCERTAIN) ?
        VBE_TRACE_CAPTURE_AUTHORITATIVE : VBE_TRACE_CAPTURE_PARTIAL;
    ...
    header.validation_flags = VALIDATION_RECORDS_OK | VALIDATION_POST_STABLE |
        ...
        (header.affine_capture_quality == VBE_TRACE_CAPTURE_AUTHORITATIVE ? VALIDATION_AFFINE_AUTHORITY : 0u) |
        (header.panel_capture_quality == VBE_TRACE_CAPTURE_AUTHORITATIVE ? VALIDATION_PANEL_AUTHORITY : 0u);
    header.reserved[0] = header.reserved[1] = header.reserved[2] = 0u;
