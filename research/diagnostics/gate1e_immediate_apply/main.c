...
static volatile uint32_t g_control_state, g_control_epoch, g_control_tid, g_control_runs;
...
static void record_event(uint32_t node, uint32_t phase, int plane,
    ...
    if (__atomic_load_n(&g_control_state, __ATOMIC_ACQUIRE) != VBE_G1E_CONTROL_RECORD) return;
    tid = thread_id();
...
static int observe(uint32_t node, volatile uint32_t *depth,
    ...
    int ret;
    if (__atomic_load_n(&g_control_state, __ATOMIC_ACQUIRE) == VBE_G1E_CONTROL_RECORD && d != 1u)
...
int vbeG1eImmediateReplayCanonical(void) {
    StageBEntry fn;
    uint32_t expected = VBE_G1E_CONTROL_IDLE;
    ...
    if ((__atomic_load_n(&g_owned, __ATOMIC_ACQUIRE) & VBE_G1E_OBSERVER_MASK) != VBE_G1E_OBSERVER_MASK ||
        ...
        g_export_entry == 0u)
        return VBE_G1E_IMMEDIATE_ERR_NOT_READY;

    if (!__atomic_compare_exchange_n(&g_control_state, &expected, VBE_G1E_CONTROL_PREPARE, 0,
                                     __ATOMIC_ACQ_REL, __ATOMIC_RELAXED))
        return VBE_G1E_IMMEDIATE_ERR_BUSY;

    zero_bytes(g_events, sizeof(g_events));
    __atomic_store_n(&g_event_count, 0u, __ATOMIC_RELAXED);
    __atomic_store_n(&g_concurrent, 0u, __ATOMIC_RELAXED);
    __atomic_store_n(&g_overlap, 0u, __ATOMIC_RELAXED);
    __atomic_store_n(&g_replay_attempted, 1u, __ATOMIC_RELAXED);
    __atomic_store_n(&g_ret_p0, VBE_G1E_SKIPPED_RETURN, __ATOMIC_RELAXED);
    __atomic_store_n(&g_ret_p1, VBE_G1E_SKIPPED_RETURN, __ATOMIC_RELAXED);
    __atomic_and_fetch(&g_flags, ~(VBE_G1E_FLAG_REPLAY_COMPLETE | VBE_G1E_FLAG_EVENT_OVERFLOW), __ATOMIC_RELAXED);
    __atomic_store_n(&g_control_tid, thread_id(), __ATOMIC_RELAXED);
    __atomic_add_fetch(&g_control_epoch, 1u, __ATOMIC_RELAXED);
    __atomic_add_fetch(&g_control_runs, 1u, __ATOMIC_RELAXED);
    __atomic_store_n(&g_control_state, VBE_G1E_CONTROL_RECORD, __ATOMIC_RELEASE);

    fn = (StageBEntry)(uintptr_t)g_export_entry;
    r0 = fn(0, (const SceIftuCscParams *)(const void *)g_identity_b);
    __atomic_store_n(&g_ret_p0, r0, __ATOMIC_RELAXED);
    if (r0 >= 0) {
        r1 = fn(1, (const SceIftuCscParams *)(const void *)g_identity_b);
        __atomic_store_n(&g_ret_p1, r1, __ATOMIC_RELAXED);
    }

    __atomic_store_n(&g_control_state, VBE_G1E_CONTROL_IDLE, __ATOMIC_RELEASE);
...
int vbeG1eImmediateGetStatus(VbeG1eObserverStatus *out) {
    ...
    uint32_t state, i, n;
    int ret;
    if (!out) return VBE_G1E_IMMEDIATE_ERR_STATE;
...
int module_start(SceSize argc, const void *args) {
    ...
    g_control_state = VBE_G1E_CONTROL_IDLE;
    g_control_epoch = g_control_tid = g_control_runs = 0u;
