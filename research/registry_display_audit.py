...
def main():
 ...
 names=load_nid_names(a.nid_db_root);rows=[];fail=[]
 for p in sorted(a.elf_dir.glob('*.elf')):
  ...
  try: imports=e.imports()
   ...
   imports=[];fail.append({'file':p.name,'module':e.modinfo.get('name',''),'error':str(ex)})
  for lib in imports:
  ...
  if mm or reg:rows.append({'file':p.name,'module':e.modinfo.get('name',''),'matched_display_terms':mm,'regmgr_imports':reg})
 result={'schema':2,'firmware':'3.65','kernel_modules_with_registry_or_color_terms':rows,'import_table_decode_failures':fail,'terms_searched':[x.decode() for x in TERMS],'scope':'decrypted retail-3.65 os0 kernel ELFs only','limitations':['registry.db0 defaults and user-mode Settings/AVConfig binaries are outside this os0-only pass unless present in os0:/kd; absence here is not a topology-exhaustion result']}
 a.json.write_text(json.dumps(result,indent=2)+'\n')
 print('REGISTRY_DISPLAY_CONSUMERS')
 print(f'  import_table_decode_failures={len(fail)}')
