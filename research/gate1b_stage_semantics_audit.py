...
def main():
    ...
    dr=Reachability(disp,disp.exports(),disp.imports(),extra_starts=[GENERATOR]+DISPLAY_MODE_ROUTINES)
    ex={}
    ...
    result={'schema':2,'firmware':'3.65','lowio_sha':low.sha256,'display_sha':disp.sha256,'lowio_exports':ex,'display_exports':dex,'lowio_mem_disp_0x100_refs':mem100_refs(lr),'display_focus':focus,'display_iftu_csc_calls':callsites_to(dr,csc_stub),'generator_parents':parents.get(GENERATOR,[])}
    ...
    for v in DISPLAY_MODE_ROUTINES:
        f=focus[f'display_internal_{v:08X}'];print(f'DISPLAY_MODE_ROUTINE 0x{v:08X}..0x{f["end"]:08X}')
