...
def main():
    ...
    for f in libs[0]['functions']:
        ...
        exports.append({'nid':f['nid'],'name':name,'va':f['va'],'thumb':f['thumb'],'category':category(name,iftu),'instruction_count':cfg.instruction_count(),'calls':calls,'iftu_calls':iftu,'absolute_state_or_mmio_constants':relevant_abs(disp,cfg)})
    intern=[]
    ...
    for x in exports:
        if x['nid']==0x8D79D187:
            print(f"OFFICIAL_DISPLAY_COLOR_SPACE_API 0x8D79D187 -> 0x{x['va']:08X} calls={[(c.get('name'),hex(c['target'])) for c in x['calls']]}")
