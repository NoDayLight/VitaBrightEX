...
FOCUS_INTERNALS={
    ...
    0x8100726C:'color_state_helper_726c',
...
def record_cfg(disp,reach,stubs,parents,va,label):
    ...
    if cfg is None:
        cfg=FunctionCFG(disp,va,True,reach.import_stubs,boundary_index=reach.boundaries,noreturn_stubs=reach.noreturn_stubs);key=(va,True)
    meta=cfg.compact(False)
    return {'label':label,'start':cfg.start,'logical_end':cfg.logical_end,'exidx_range':meta['exidx_range'],'exidx_exact':cfg.exidx_exact,'boundary_sources':cfg.boundary_sources,'termination_reason':meta['termination_reason'],'instruction_count':cfg.instruction_count(),'parents':parents.get(key,[]),'calls':call_semantics(cfg,stubs),'absolute_state_or_mmio_constants':relevant_abs(disp,cfg),'instructions':[{'va':i.address,'text':ins_text(i)} for i in all_insns(cfg)]}
...
def main():
    ...
    names=load_nid_names(a.nid_db_root);reach=Reachability(disp,disp.exports(),disp.imports(),extra_starts=FOCUS_INTERNALS.keys());stubs=import_stub_map(disp,names);parents=function_parents(reach)
    ...
    for f in libs[0]['functions']:
        cfg=reach.functions.get((f['va'],f['thumb'])) or FunctionCFG(disp,f['va'],f['thumb'],reach.import_stubs,boundary_index=reach.boundaries,noreturn_stubs=reach.noreturn_stubs)
        calls=call_semantics(cfg,stubs);iftu=[c for c in calls if c.get('nid') in IFTU_NIDS or c.get('library')=='SceIftuForDriver']
        name=best_name(names,f['nid']);meta=cfg.compact(False)
        row={'nid':f['nid'],'name':name,'va':f['va'],'thumb':f['thumb'],'category':category(name,iftu),'logical_end':cfg.logical_end,'exidx_range':meta['exidx_range'],'exidx_exact':cfg.exidx_exact,'boundary_sources':cfg.boundary_sources,'termination_reason':meta['termination_reason'],'instruction_count':cfg.instruction_count(),'calls':calls,'iftu_calls':iftu,'absolute_state_or_mmio_constants':relevant_abs(disp,cfg)}
    ...
    for key,cfg in sorted(reach.functions.items()):
        ...
        meta=cfg.compact(False)
        intern.append({'start':cfg.start,'logical_end':cfg.logical_end,'exidx_range':meta['exidx_range'],'exidx_exact':cfg.exidx_exact,'boundary_sources':cfg.boundary_sources,'termination_reason':meta['termination_reason'],'mode':'thumb' if cfg.thumb else 'arm','instruction_count':cfg.instruction_count(),'iftu_calls':iftu,'parents':parents.get(key,[]),'absolute_state_or_mmio_constants':relevant_abs(disp,cfg),'instructions':[{'va':i.address,'text':ins_text(i)} for i in all_insns(cfg)] if len(iftu)>=2 else None})
    dual=[]
    for x in intern:
    ...
    for nid,label in FOCUS_EXPORTS.items():
        ...
        focus_exports[label]={'nid':nid,'va':f['va'],'logical_end':cfg.logical_end,'calls':row['calls'],'absolute_state_or_mmio_constants':row['absolute_state_or_mmio_constants'],'instructions':[{'va':i.address,'text':ins_text(i)} for i in all_insns(cfg)]}
    color=focus_exports['ksceDisplaySetColorSpaceMode'];color_targets=[c['target'] for c in color['calls'] if c.get('internal')]
    result={'schema':3,'firmware':'3.65','analysis_authority':'validated-logical-boundaries','elf_sha256':{'SceDisplay':disp.sha256,'SceLowio':low.sha256},'SceDisplayForDriver_library_nid':libs[0]['library_nid'],'export_count':len(exports),'exports':sorted(exports,key=lambda x:x['va']),'iftu_reachable_internal_functions':intern,'dual_private_csc_setter_internal_candidates':dual,'shared_display_state_constants':shared,'focus_exports':focus_exports,'focus_internal_functions':focus,'official_color_space_path':{'export_nid':0x8D79D187,'export_va':export_by_nid[0x8D79D187][0]['va'],'internal_targets':color_targets,'direct_iftu_calls':[c for c in color['calls'] if c.get('library')=='SceIftuForDriver'],'interpretation':'Official SceDisplay color-space state path; do not conflate with ksceLcdSetDisplayColorSpaceMode until the internal helper dataflow is correlated.'},'evidence_notes':['semantic names are from pinned vita-headers db/360 and retail 3.65 NIDs are taken from the ELF','unknown functions are not named speculatively','absolute addresses are discovery/state-clustering evidence, not automatically hardware registers','function-local claims use validated logical boundaries rather than exidx ranges alone']}
    a.json.write_text(json.dumps(result,indent=2)+'\n')
    print('DISPLAY_FOR_DRIVER_TOPOLOGY');print(f'  retail export count={len(exports)}')
    for x in sorted(exports,key=lambda x:x['va']):
        cs=','.join(c.get('name') or f"0x{c.get('nid',0):08X}" for c in x['iftu_calls']) or '-'
        print(f"  0x{x['nid']:08X} {x['name'] or 'UNKNOWN'} -> 0x{x['va']:08X}..0x{x['logical_end']:08X} [{x['category']}] iftu={cs}")
    ...
    for label,x in focus.items():print(f"  FOCUS {label} 0x{x['start']:08X}..0x{x['logical_end']:08X} term={x['termination_reason']} calls={[(c.get('name'),hex(c['target'])) for c in x['calls']]}")
