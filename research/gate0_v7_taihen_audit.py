...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--patches',type=Path,required=True);ap.add_argument('--header',type=Path,required=True);ap.add_argument('--text',type=Path,required=True);a=ap.parse_args();p=a.patches.read_text();h=a.header.read_text()
 rel=re.search(r'int tai_hook_release\(.*?\n\}',p,re.S);need(rel,'tai_hook_release missing');need('hooks_remove_hook' in rel.group() and 'slab_free(slab, hook)' in rel.group(),'release does not prove unlink + immediate slab free')
