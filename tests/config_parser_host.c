...
int main(void){int failures=0;VitaBrightConfig cfg;
 failures+=ok(parse_file("vitabrightex.cfg",&cfg)==0&&cfg.filter_cct==6500&&cfg.filter_gamma==1.0f,"source config");
 ...
 failures+=ok(parse_text("lcd_color_space_mode=1\nlcd_saturation_boost=0\nlcd_ips_enhance=1\ndisplay_color_space_mode=0\n",&cfg)==0&&cfg.display_color_space_mode==0&&cfg.display_color_space_scope==VBE_COLOR_SPACE_SCOPE_ANY_PANEL,"color-space alias pair uses last valid occurrence");
