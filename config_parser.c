...
#define APPLY_INT(name, field) \
    if (streq(key, name)) { \
        if (parse_int_exact(value, &ivalue) < 0) return -1; \
        cfg->field = ivalue; \
        return 0; \
    }
#define APPLY_FLOAT(name, field) \
    if (streq(key, name)) { \
        if (parse_float_exact(value, &fvalue) < 0) return -1; \
        cfg->field = fvalue; \
        return 0; \
    }
    ...
    APPLY_INT("display_color_space_mode", lcd_color_space_mode)
    APPLY_INT("lcd_color_space_mode", lcd_color_space_mode)
    APPLY_INT("lcd_rgb_range_mode", lcd_rgb_range_mode)
    APPLY_INT("lcd_saturation_boost", lcd_saturation_boost)
    APPLY_INT("lcd_ips_enhance", lcd_ips_enhance)
...
static int end_line(VbeConfigParser *parser) {
    ...
    } else if (parser->state == CFG_VALUE) {
        ret = finish_value(parser);
    }
    if (ret < 0) {
        parser->failed = 1;
        return -1;
...
int vbe_config_parser_feed(VbeConfigParser *parser, unsigned char raw) {
    ...
    case CFG_START:
        if (c == ' ' || c == '\t') return 0;
        if (c == '#' || c == ';') {
            parser->state = CFG_COMMENT;
            return 0;
        }
        if (is_key_char(c)) {
            parser->key[0] = (char)c;
            parser->key_len = 1;
            parser->state = CFG_KEY;
            return 0;
        }
        parser->state = CFG_IGNORE;
        return 0;

    case CFG_KEY:
        if (is_key_char(c)) {
            if (parser->key_len >= VBE_CONFIG_KEY_MAX - 1) {
                parser->state = CFG_IGNORE;
                return 0;
            }
            parser->key[parser->key_len++] = (char)c;
            return 0;
        }
        if (c == ' ' || c == '\t') {
            parser->state = CFG_AFTER_KEY;
            return 0;
        }
    ...
    case CFG_BEFORE_VALUE:
        if (c == ' ' || c == '\t') return 0;
        parser->value[0] = (char)c;
        parser->value_len = 1;
        parser->state = CFG_VALUE;
        return 0;

    case CFG_VALUE:
        if (parser->value_len >= VBE_CONFIG_VALUE_MAX - 1) {
            parser->failed = 1;
            return -1;
        }
        parser->value[parser->value_len++] = (char)c;
        return 0;
