...
static int key_known(const char *key) {
    static const char *const keys[] = {
        "oled_panel_lut_override", "panel_lut_path", "color_r_bias",
        "color_g_bias", "color_b_bias", "night_mode_enabled",
        "night_mode_threshold", "display_color_space_mode",
        "lcd_color_space_mode", "lcd_rgb_range_mode",
        "lcd_saturation_boost", "lcd_ips_enhance", "oled_dim_workaround",
...
static int parse_int_exact(const char *s, int *out) {
    ...
    unsigned long long value = 0; int saturated = 0;
    while (*s >= '0' && *s <= '9') {
        unsigned int digit = (unsigned int)(*s - '0');
        if (!saturated) {
            if (value > (limit - digit) / 10ull) { value = limit; saturated = 1; }
...
void vbe_config_defaults(VitaBrightConfig *out) {
    VitaBrightConfig defaults = {
        .oled_panel_lut_override = 0, .panel_lut_path = "", .oled_dim_workaround = 1,
        .color_r_bias = 0, .color_g_bias = 0, .color_b_bias = 0,
        .night_mode_enabled = 0, .night_mode_threshold = 6,
        .display_color_space_mode = 0,
        .display_color_space_scope = VBE_COLOR_SPACE_SCOPE_ANY_PANEL,
        .lcd_rgb_range_mode = 0,
        .filter_cct = 6500, .filter_gamma = 1.0f, .filter_contrast = 1.0f,
        .filter_brightness = 0.0f, .filter_invert = 0, .filter_panel_enhance = 0,
    };
    *out = defaults;
}
...
static int copy_path(char *dst, int dst_size, const char *src) {
    int len = 0; while (src[len]) ++len; if (len >= dst_size) return -1;
    for (int i = 0; i <= len; ++i) dst[i] = src[i]; return 0;
}
...
static int apply_value(VitaBrightConfig *cfg, const char *key, const char *value) {
    int ivalue = 0; float fvalue = 0.0f;
...
#define APPLY_INT(name, field) if (streq(key, name)) { if (parse_int_exact(value, &ivalue) < 0) return -1; cfg->field = ivalue; return 0; }
#define APPLY_FLOAT(name, field) if (streq(key, name)) { if (parse_float_exact(value, &fvalue) < 0) return -1; cfg->field = fvalue; return 0; }
...
static void reset_line(VbeConfigParser *parser) {
    parser->key_len = 0; parser->value_len = 0; parser->known_key = 0; parser->state = CFG_START;
}
void vbe_config_parser_init(VbeConfigParser *parser, VitaBrightConfig *out) {
    vbe_config_defaults(out); parser->out = out; vbe_text_newline_init(&parser->newline);
    parser->failed = 0; reset_line(parser);
}
static int begin_value(VbeConfigParser *parser) {
    parser->key[parser->key_len] = '\0'; parser->known_key = key_known(parser->key);
    parser->value_len = 0; parser->state = parser->known_key ? CFG_BEFORE_VALUE : CFG_IGNORE; return 0;
}
static int finish_value(VbeConfigParser *parser) {
    while (parser->value_len > 0 && (parser->value[parser->value_len - 1] == ' ' || parser->value[parser->value_len - 1] == '\t')) --parser->value_len;
    parser->value[parser->value_len] = '\0';
    if (apply_value(parser->out, parser->key, parser->value) < 0) { parser->failed = 1; return -1; }
    return 0;
}
static int end_line(VbeConfigParser *parser) {
    int ret = 0;
    if (parser->state == CFG_BEFORE_VALUE) { parser->value[0] = '\0'; ret = apply_value(parser->out, parser->key, parser->value); }
    else if (parser->state == CFG_VALUE) ret = finish_value(parser);
    if (ret < 0) { parser->failed = 1; return -1; }
    reset_line(parser); return 0;
}
int vbe_config_parser_feed(VbeConfigParser *parser, unsigned char raw) {
    if (parser->failed) return -1;
    unsigned char c = 0; int decoded = vbe_text_newline_feed(&parser->newline, raw, &c);
    if (decoded < 0) { parser->failed = 1; return -1; }
    if (decoded == 0) return 0; if (c == '\n') return end_line(parser);
    switch (parser->state) {
    case CFG_START:
        if (c == ' ' || c == '\t') return 0;
    ...
    case CFG_KEY:
        if (is_key_char(c)) { if (parser->key_len >= VBE_CONFIG_KEY_MAX - 1) { parser->state = CFG_IGNORE; return 0; } parser->key[parser->key_len++] = (char)c; return 0; }
        if (c == ' ' || c == '\t') { parser->state = CFG_AFTER_KEY; return 0; }
        if (c == '=') return begin_value(parser); parser->state = CFG_IGNORE; return 0;
    case CFG_AFTER_KEY:
        if (c == ' ' || c == '\t') return 0; if (c == '=') return begin_value(parser); parser->state = CFG_IGNORE; return 0;
    case CFG_BEFORE_VALUE:
        if (c == ' ' || c == '\t') return 0; parser->value[0] = (char)c; parser->value_len = 1; parser->state = CFG_VALUE; return 0;
    case CFG_VALUE:
        if (parser->value_len >= VBE_CONFIG_VALUE_MAX - 1) { parser->failed = 1; return -1; }
        parser->value[parser->value_len++] = (char)c; return 0;
    case CFG_IGNORE: case CFG_COMMENT: return 0;
    default: parser->failed = 1; return -1;
    }
}
int vbe_config_parser_finish(VbeConfigParser *parser) {
    if (parser->failed || vbe_text_newline_finish(&parser->newline) < 0) { parser->failed = 1; return -1; }
    if (parser->state != CFG_START && end_line(parser) < 0) return -1;
    validate(parser->out); return 0;
}
