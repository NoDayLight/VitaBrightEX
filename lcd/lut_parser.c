...
enum { LCD_START = 0, LCD_VALUE, LCD_AFTER_VALUE, LCD_COMMENT };

static int is_space(unsigned char c) { return c == ' ' || c == '\t'; }
static int is_comment(unsigned char c) { return c == '#' || c == ';'; }
...
int vbe_lcd_lut_parser_feed(VbeLcdLutParser *parser, unsigned char raw) {
    ...
    case LCD_START:
        ...
        if (is_comment(c)) { parser->state = LCD_COMMENT; return 0; }
        if (c >= '0' && c <= '9') {
    ...
    case LCD_VALUE:
        ...
        if (is_space(c)) { parser->state = LCD_AFTER_VALUE; return 0; }
        if (is_comment(c)) {
            if (commit_value(parser) < 0) return -1;
